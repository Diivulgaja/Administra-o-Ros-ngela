window.AdminAvaliacoes = (() => {
  ...
  async function moderate(id, patch) {
    try {
      ...
      notify({ title: 'Avaliação atualizada', message: 'A moderação foi salva com sucesso.', variant: 'success' });
    } catch (error) {
      notify({ title: 'Falha ao moderar', message: error.message || 'Não foi possível atualizar a avaliação.', variant: 'error' });
    }
  }
